...
abstract class QueryBuilderTest extends DatabaseTestCase
{
    ...
    public function conditionProvider()
    {
        $conditions = [
            ...
            [
                ['not like', 'name', ['heyho', 'abc']],
                '[[name]] NOT LIKE :qp0 AND [[name]] NOT LIKE :qp1',
                [':qp0' => '%heyho%', ':qp1' => '%abc%']
            ],
            [
                ['or like', 'name', ['heyho', 'abc']],
                '[[name]] LIKE :qp0 OR [[name]] LIKE :qp1',
                [':qp0' => '%heyho%', ':qp1' => '%abc%']
            ],
            [
                ['or not like', 'name', ['heyho', 'abc']],
            ...
            [
                ['like', 'name', new Expression('CONCAT("test", colname, "%")')],
                '[[name]] LIKE CONCAT("test", colname, "%")',
                []
            ],
            [
                ['not like', 'name', new Expression('CONCAT("test", colname, "%")')],
                '[[name]] NOT LIKE CONCAT("test", colname, "%")',
                []
            ],
            [
                ['or like', 'name', new Expression('CONCAT("test", colname, "%")')],
                '[[name]] LIKE CONCAT("test", colname, "%")',
                []
            ],
            [
                ['or not like', 'name', new Expression('CONCAT("test", colname, "%")')],
                '[[name]] NOT LIKE CONCAT("test", colname, "%")',
                []
            ],
            [
                ['like', 'name', [new Expression('CONCAT("test", colname, "%")'), 'abc']],
                '[[name]] LIKE CONCAT("test", colname, "%") AND [[name]] LIKE :qp0',
                [':qp0' => '%abc%']
            ],
            [
                ['not like', 'name', [new Expression('CONCAT("test", colname, "%")'), 'abc']],
                '[[name]] NOT LIKE CONCAT("test", colname, "%") AND [[name]] NOT LIKE :qp0',
                [':qp0' => '%abc%']
            ],
            [
                ['or like', 'name', [new Expression('CONCAT("test", colname, "%")'), 'abc']],
                '[[name]] LIKE CONCAT("test", colname, "%") OR [[name]] LIKE :qp0',
                [':qp0' => '%abc%']
            ],
            [
                ['or not like', 'name', [new Expression('CONCAT("test", colname, "%")'), 'abc']],
    ...
    public function existsParamsProvider()
    {
        return [
            [
                'exists',
                $this->replaceQuotes("SELECT [[id]] FROM [[TotalExample]] [[t]] WHERE EXISTS (SELECT [[1]] FROM [[Website]] [[w]])")
            ],
            [
                'not exists',
                $this->replaceQuotes("SELECT [[id]] FROM [[TotalExample]] [[t]] WHERE NOT EXISTS (SELECT [[1]] FROM [[Website]] [[w]])")
            ]
        ];
    }
